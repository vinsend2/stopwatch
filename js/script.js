...
const btnStop = document.querySelector('.watch__btn-stop');
...
const startTime = () => {
  if(!active) {
    ...
    startTimer = setInterval(() => {
      ...
      mSec = (newDate / 10) % 100;
      sec = (newDate / 1000) % 60;
      min = (newDate / 1000 / 60) % 60;
      hours = (newDate / 1000 / 60 / 60) % 24;

      millSecString.textContent = (mSec < 10) ? `00${Math.floor(mSec)}` : (mSec >= 10 && mSec < 100) ?
      `0${Math.floor(mSec)}` : Math.floor(mSec);
      secString.textContent =  (sec < 10) ? `0${Math.floor(sec)}` : Math.floor(sec);
      minString.textContent = (min < 10) ? `0${Math.floor(min)}` : Math.floor(min);
      hoursString.textContent = Math.floor(hours);
    }, 10); // 10 == millSec ... :)
